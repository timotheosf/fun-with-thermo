\chapter{Ensemble Grandecanônico}
    ...
    \begin{statementbox}{Função de partição grandecanônica}
        Dado um ensemble grandecanônico, definimos sua função de partição $\zg$ como a soma dos pesos de Boltzmann sobre \textit{todos} os estados de equilíbrio possíveis ao sistema:
        \begin{equation}
            \zg(T,V,\mu) = \sum_{j}e^{-E(j)\beta+N(j)\alpha}.
        \end{equation}
        Esta função está ligada ao chamado \textit{potencial grande canônico} pela relação:
        \begin{equation}
            \mathcal{A}(T,V,\mu)=-k_BT\ln\zg\,,
        \end{equation}
        lembrando que
        \begin{equation}
            \mathcal{A}\equiv U - TS + \mu N\,.
        \end{equation}
    \end{statementbox}