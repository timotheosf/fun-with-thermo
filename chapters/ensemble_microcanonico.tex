...
\section{Encontrando a multiplicidade}
    ...
    A tarefa principal no ensemble microcanônico é determinar a multiplicidade $\Omega$ do macroestado. A partir da entropia de Boltzmann, conseguimos descrever completamente a termodinâmica do sistema. Os exercícios resolvidos mostram \textit{como} determinar a multiplicidade em diversos casos.

    Caso o sistema tenha níveis discretos de energia, esta tarefa se resume a responder à pergunta \textit{de quantas formas possíveis podemos distribuir a energia $U$ entre as $N$ partículas?} Em outras palavras, um problema de física se torna um problema de análise combinatória.
...
\section{Termodinâmica em ação}
    ...
    A partir da multiplicidade do sistema, podemos extrair todas as propriedades termodinâmicas do sistema a partir da entropia de Boltzmann. A Figura \ref{fig:maquina_da_mecanica_estatistica} ilustra o processo: basta \textit{girar a manivela}.
    ...
    \begin{solved}{Entropia do paramagneto de dois estados}{}
        ...
        \tcblower
        \noindent\textbf{Resolução:}\\
        \indent Temos \[S(U,N)=k_B\ln(\Omega)=k_B\left[\ln(N!)+\ln\left(\frac{N}{2}+\frac{U}{2\upmu_m\mathrm{H}}\right)!+\ln\left(\frac{N}{2}-\frac{U}{2\upmu_m\mathrm{H}}\right)!\right].\] No limite termodinâmico, podemos usar a fórmula de Stirling (cf. Eq. \eqref{eq_appx:approximacao_de_stirling}), com $\ppu=U/N$ fixo:
    \end{solved}
...
\starredsection{Espectro contínuo de energia}
